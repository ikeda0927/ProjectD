...
public class Bullet : MonoBehaviour
{
    ...
    public float force = 1000f;//爆発の威力
    public float range = 20f;//爆発の範囲
    GameObject effect;
    ...
    void Update()
    {
        if (((gameObject != null && !particleJudge && gameObject.transform.position.y <= .8f) || collisionWithTarget) && !called)
        {
            //Debug.Log("Explosion");
            called = true;
            Explosion();
        }
        else if (gameObject != null && particleJudge && !particle.isPlaying)// && !particle.isPlaying
        {
            Destroy(gameObject);
        }
    ...
    void Explosion()
    {
        ...
        foreach (Collider other in others)
        {
            ...
            if (other.tag == "Bullet" || other.tag == "Target" || other.tag == "Tank" || other.tag == "Battery" || other.tag == "Caterpiller1" || other.tag == "Caterpiller2")
            {
                ...
                {
                    other.GetComponentInParent<HP_Contoroller>().LoseHP(1);
                }
            }
        }
    }
